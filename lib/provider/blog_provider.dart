...
class BlogProvider with ChangeNotifier {
  ...
  Future getPost() async {
    final response = await get(Uri.parse(HttpService.blogs), headers: {
  ...
  Future<Map<String, dynamic>> savePost(
      String title, String body, String author, String token) async {
  ...
  Future<Map<String, dynamic>> updatePost(
      String title, String body, int id, String token) async {
    final Map<String, dynamic> bodyPost = {'title': title, 'body': body};

    _loadingCircle = Status.loading;
    notifyListeners();

    final response = await put(
            Uri.parse(HttpService.updateblog + id.toString()),
            headers: {
              'content-Type': 'application/json',
              'Authorization': 'Bearer $token'
            },
  ...
  Future<Map<String, dynamic>> deletePost(
      String author, int id, String token) async {
    print(id);
    final Map<String, dynamic> bodyPost = {'author': author};

    _loadingCircle = Status.loading;
    notifyListeners();

    final response = await delete(
            Uri.parse(HttpService.deleteblog + id.toString()),
            headers: {
              'content-Type': 'application/json',
              'Authorization': 'Bearer $token'
            },
